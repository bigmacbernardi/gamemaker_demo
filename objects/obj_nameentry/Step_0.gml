...
if defaulted {
	...
	if enter_key{
		if index2== array_length(characters)//on "Submit"
			saveName();
		else if string_length(name_so_far) == char_limit
			 name_so_far = string_copy(name_so_far, 1,char_limit-1) + characters[index2]
		else name_so_far = name_so_far + characters[index2];
	}
	else{
		if left_key{
			if index= array_length(characters)
			if (--index2<0)index2=array_length(characters)-1;
		...
		if right_key{
			if index2==9 index=array_length(characters);
			else if (++index2>=array_length(characters))characters=0;
		}
		if down_key{
