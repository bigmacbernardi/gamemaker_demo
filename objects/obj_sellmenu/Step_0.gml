...
if (!frameHasPassed) frameHasPassed = true;
	...
	if (_moveV != 0){
		...
		else if (mode == 1) and index2==0{
			numToSell=min(options[|index][0].numOwned,max(numToSell-_moveV,0));;			
			//global.targets = [global.currentParty[index2]];
		}
	}
	if (_moveH!= 0){ //will wrap eventually
			if mode==0{
				...
				if (index2 < 0)
					...
					index2 = 0;
				//global.targets = [global.currentParty[index2]];//only if doin equip thingky
			}
	}
}
