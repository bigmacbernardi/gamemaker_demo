...
for (var i = 0; i < 4; i++){
  ...
  if (global.currentParty[i]==noone) draw_text_transformed(drawX,drawY,"[empty]",.6,.6,0);
	...
	draw_sprite_ext(front_sprites[global.currentParty[i]],0,drawX+50,drawY+10,.5,.5,0,c_white,alpha);
...
for (var i = scrollLevel; i < min(8+scrollLevel,ds_list_size(options)); i++){
  if (i!=index2) draw_sprite_ext(front_sprites[options[|i]],0,drawX,drawY+23,1,1,0,c_dkgray,.8); //do different indicator
